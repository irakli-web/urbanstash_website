                  ...
                  {[
                    {
                      ...
                      avatar: 'F',
                      time: '1 hour ago',
                      text: 'Happy to be your extended home!! @urbanstash_ thanks for holding my thingssssss ⭐',
                      tag: '@urbanstash_',
                      img: 'https://images.unsplash.com/photo-1600585152915-d208bec867a1?w=400&h=600&fit=crop&q=80',
                    },
                    {
                      handle: 'milesnyc_',
                      avatar: 'M',
                      time: '2 days ago',
                      text: 'Finally cleared out my whole closet. Couldn't have done it without @urbanstash 🙌',
                      tag: '@urbanstash',
                    ...
                    },
                    {
                      ...
                      avatar: 'P',
                      time: '5 days ago',
                      text: 'Unit is spotless and SO close to my apartment. 10/10 recommend @urbanstash_',
                      tag: '@urbanstash_',
                      img: 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=400&h=600&fit=crop&q=80',
                    },
                    {
                      handle: 'jakebrooks.co',
                      avatar: 'J',
                      time: '1 week ago',
                      text: 'Moving between apartments? @urbanstash saved my whole move. Literally a lifesaver 📦',
                      tag: '@urbanstash',
                      img: 'https://images.unsplash.com/photo-1600585152220-90363fe7e115?w=400&h=600&fit=crop&q=80',
                    },
                  ].map((post, i) => (
                    <div
                      key={i}
                      className="flex-shrink-0 w-52 rounded-2xl overflow-hidden glass-card border border-white/[0.08] group"
                      style={{ scrollSnapAlign: 'start' }}
                    >
                      {/* Post image */}
                      <div className="relative h-64 overflow-hidden">
                        <img src={post.img} alt={post.handle} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                        {/* Instagram-style overlay header */}
                        <div className="absolute top-0 left-0 right-0 p-3 bg-gradient-to-b from-black/60 to-transparent flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-accent flex items-center justify-center text-[10px] font-black text-on-accent flex-shrink-0">
                            {post.avatar}
                          </div>
                          <div className="min-w-0">
                            <p className="text-white text-[10px] font-bold leading-none truncate">{post.handle}</p>
                            <p className="text-white/60 text-[9px] leading-none mt-0.5">{post.time}</p>
                          </div>
                          {/* Instagram icon */}
                          <svg className="w-4 h-4 text-white/70 ml-auto flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                          <p className="text-white text-[10px] leading-snug line-clamp-3">{post.text}</p>
                        </div>
                      </div>
                      {/* Tag footer */}
