...
import { sizes, whyStashItems, categoryMeta } from '../data/sizes';
...
export default function SizingPage() {
  ...
  // Reset to image whenever the selected size changes
  useEffect(() => {
    setMediaMode('image');
  }, [activeIndex]);
  ...
  return (
    <div className="min-h-screen bg-theme text-white">
      ...
      <main>
        {/* Hero */}
        <section className="hero-section relative py-24 md:py-32 px-4 md:px-6 overflow-hidden">
          ...
          <div className="relative max-w-5xl mx-auto text-center">
            ...
            <p className="hero-subtitle text-zinc-400 text-lg md:text-xl mb-12 max-w-lg mx-auto leading-relaxed">
        ...
        <section className="bg-charcoal py-20 md:py-24 px-4 md:px-6 border-t border-white/[0.04] relative overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto">
            ...
            <h2 className="section-title text-3xl md:text-4xl font-black uppercase tracking-tight text-center mb-14">
            ...
            <div className="grid lg:grid-cols-12 gap-8 lg:gap-12">
              ...
              <div className="lg:col-span-7 xl:col-span-8 flex flex-col">
                ...
                <div className="flex items-center gap-3 mb-3">
                  <span className="px-3 py-1 rounded-full bg-accent/10 text-accent text-xs font-bold uppercase">{active.category}</span>
                  <span className="text-sm text-zinc-500 font-mono">{active.dimensions}</span>
                </div>
                <h3 className="text-xl md:text-2xl font-bold text-white mb-2">{active.for}</h3>
                <p className="text-zinc-400 leading-relaxed mb-5">{SIZE_DESCRIPTION[active.category]}</p>

                {/* Media viewer */}
                <div className="relative mb-6" style={{ aspectRatio: '16/9' }}>
                  ...
                  <div className="absolute inset-0 rounded-2xl overflow-hidden border border-white/[0.08]">
                    ...
                    {mediaMode === 'image' && (
                      <>
                        <img
                          src={meta.items[0].img}
                          alt={`${active.category} unit`}
                          className="w-full h-full object-cover"
                        />
                        <div className="absolute inset-0 bg-black/25" />

                        {/* Watch video CTA — bottom left */}
                        <button
                          onClick={() => setMediaMode('video')}
                          className="absolute bottom-4 left-4 flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-black/60 backdrop-blur-md border border-white/20 hover:bg-black/80 hover:border-white/40 transition-all group"
                        >
                          <span className="w-7 h-7 rounded-full bg-accent flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                            <svg className="w-3.5 h-3.5 text-on-accent ml-0.5" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M8 5v14l11-7z" />
                            </svg>
                          </span>
                          <span className="text-white text-xs font-semibold">{meta.videoLabel}</span>
                        </button>
                      </>
                    )}
                    ...
                    {mediaMode === 'video' && (
                      <iframe
                        src={meta.videoUrl}
                        title={`${active.category} unit tour`}
                  ...
                  <button
                    ...
                    className={`absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 z-10 w-10 h-10 rounded-full border flex items-center justify-center transition-all shadow-lg ${
                      mediaMode === 'image'
                  ...
                  <button
                    onClick={() => setMediaMode('video')}
                    className={`absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 z-10 w-10 h-10 rounded-full border flex items-center justify-center transition-all shadow-lg ${
                      mediaMode === 'video'
                        ? 'bg-accent border-accent text-on-accent'
                        : 'bg-zinc-900 border-white/10 text-zinc-400 hover:border-white/30 hover:text-white'
                    }`}
                  ...
                  <div className="absolute bottom-3 right-4 flex items-center gap-1.5 z-10">
                    <button
                      onClick={() => setMediaMode('image')}
                      className={`h-1.5 rounded-full transition-all duration-300 ${mediaMode === 'image' ? 'w-5 bg-white' : 'w-1.5 bg-white/30'}`}
                    />
                    <button
                      onClick={() => setMediaMode('video')}
                      className={`h-1.5 rounded-full transition-all duration-300 ${mediaMode === 'video' ? 'w-5 bg-white' : 'w-1.5 bg-white/30'}`}
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <Link
                    to="/units"
                    className="cta-btn inline-flex items-center gap-2 px-7 py-3 bg-accent text-on-accent font-bold rounded-full hover:opacity-90"
                  >
                    Find {active.category.toLowerCase()} units
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                    </svg>
                  </Link>
                  <Link to="/calculator" className="text-zinc-500 hover:text-accent text-sm font-medium transition-colors">
                    Not sure? Try the calculator →
                  </Link>
                </div>
              </div>
            </div>
        ...
        <section className="bg-charcoal py-20 md:py-24 px-4 md:px-6 border-t border-white/[0.04]">
          <div className="max-w-5xl mx-auto overflow-x-auto">
            ...
            <h2 className="section-title text-3xl md:text-4xl font-black uppercase tracking-tight text-center mb-12">
        ...
        <section className="bg-accent-tint py-24 px-4 md:px-6 border-t border-white/[0.04] relative overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto">
            ...
            <h2 className="section-title text-3xl md:text-4xl font-black uppercase tracking-tight text-center mb-14 text-white">
